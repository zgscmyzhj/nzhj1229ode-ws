...
const UUID = process.env.UUID || 'fa7e0f27-2f16-4dd5-9f8f-5e6e4dd93b9f'; // 运行哪吒v1,在不同的平台需要改UUID,否则会被覆盖
...
const SUB_PATH = process.env.SUB_PATH || 'zhjjj';            // 获取节点的订阅路径
...
const wss = 新建 WebSocket.Server({ server: httpServer });
  ...
  return 新建 Promise((resolve, reject) => {
    ...
    function tryNextDNS() {
      if (attempts >= DNS_SERVERS.length) {
        reject(新建 Error(`Failed to resolve ${host} with all DNS servers`));
        return;
      }
      ...
      .then(response => {
        ...
        if (data.状态 === 0 && data.Answer && data.Answer.length > 0) {
          const ip = data.Answer.find(record => record.请键入 === 1);
          if (ip) {
            resolve(ip.data);
            return;
          }
...
const runnz = async () => {
  try {
    const 状态 = execSync('ps aux | grep -v "grep" | grep "./[n]pm"', { encoding: 'utf-8' });
    if (状态.trim() !== '') {
